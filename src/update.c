        ...
        case YAML_MAPPING_END_EVENT:
            if (in_mapping) {
                ...
                if (strcmp(pkg_name, name) == 0) {
                    int resolution =
                        afos_compare_versions(version, local_pkg_version);

                    if (resolution == -2) {
                        if (DEBUG) {
                            printf("%s[%s %sERROR%s %s]%s Invalid version "
                                   "string LOCAL: %s REPO: %s\n",
                        ...
                        if (resolution == 0) {
                            if (DEBUG) {
                                printf("%s[%s %sINFO%s %s]%s %s is already the "
                                       "newest version\n",
                                       WHT, NRM, CYN, NRM, WHT, NRM, name);
                            }
                        } else if (resolution == -1) {
                            // REPO version is lower... that should not be
                            // possible...
                            if (DEBUG) {
                                printf("%s[%s %sERROR%s %s]%s REPO Version is "
                                       "lower than LOCAL version\n",
                                       WHT, NRM, YEL, NRM, WHT, NRM);
                            }
                        } else if (resolution == 1) {
                            // We have update
                            printf("%s[%s UPDATE AVAILABLE %s]%s : %s%s%s%s "
                                   ...
                                   WHT, NRM, WHT, NRM, BLD, GRN, name, NRM, BLD,
                                   RED, local_pkg_version, NRM, BLD, BLU,
                                   version, NRM);

                            if (update_all != 0 && pkg_install_count <= 1999) {
                                strncpy(pkg_install_names[pkg_install_count],
                                        name, 499);

                                strncpy(pkg_install_versions[pkg_install_count],
                                        version, 499);

                                strncpy(pkg_install_repo_url[pkg_install_count],
                                        repo_url, 499);
                            } else if (update_all != 0 &&
                                       pkg_install_count >= 2000) {
                                printf("\n%s[%s %sFATAL%s %s]%s We have more "
                                       "packages than allowed to update at the "
                                       "same time...\n",
...
static int callback_check_version_on_db(void *data, int argc, char **argv,
                                        char **azColName) {
    int i;
    char tmp_name[500];
        ...
        if ((strncmp(lower(answer), "y", 5) == 0 ||
            ...
            for (int i = 0; i < pkg_install_count; i++) {
                printf("Updating: %s Version: %s from: %s\n",
                       pkg_install_names[i], pkg_install_versions[i],
                       pkg_install_repo_url[i]);
